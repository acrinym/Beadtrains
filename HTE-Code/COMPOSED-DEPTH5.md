...
Repository: D:\github\Beadtrains
Origin: https://github.com/acrinym/Beadtrains.git
Scan HEAD: d2182a1a67fdbcdc0c4531c2e6c2abf98b2e4194
...
# RESOLVED PACK: framework.pytest

## PACK DOCUMENT: CONCERNS.md

# pytest Depth-5 Framework Concerns
...
- Identity/version: pytest version, Python interpreter and plugin set define collection, fixture and assertion behavior.
- Discovery/parser: testpaths/python_files/classes/functions, import mode and doctest/plugins determine what is collected.
- Binding/modules: fixture name resolution, conftest scope, monkeypatch and import cache can bind hidden dependencies around tests.
- Type semantics: parametrization IDs/values and dynamic Python typing can exercise types unlike static annotations suggest.
- Evaluation/control: fixture scope/autouse, yield teardown, markers, xfail/skip, parametrization and hooks shape execution graph.
- Numeric edges: approximate comparisons, timeouts and parametrized boundaries must encode tolerance/units explicitly.
- Memory/lifetime: session/module fixtures, temp dirs, threads/processes/event loops and monkeypatches must unwind at correct scope.
- Errors/recovery: collection error, fixture setup/teardown error, assertion failure, xfail/xpass and timeout represent different evidence.
- Concurrency: xdist/processes, pytest-asyncio loop scope and external shared resources can introduce ordering/race behavior.
- Serialization: xdist and cache/plugins may pickle/serialize test data; fixtures should not assume in-process object identity when distributed.
- Security: test credentials, temp files, subprocess/network fixtures and unsafe deserialization must remain isolated from real environments.
- ABI/interop: C extensions/native libraries still require interpreter/platform ABI compatibility.
- Performance: fixture scope, parametrization explosion, slow I/O and plugin overhead can hide product regressions or suite bottlenecks.
- Instrumentation: --collect-only, -vv, fixture setup output, plugin list and junit/coverage reports prove what ran.
- Failure signatures: FixtureLookupError is dependency binding; collection import error differs from assertion; leaked async tasks often event-loop ownership.
- Undefined/framework-defined: test order is not a correctness contract unless explicitly constrained by plugin/config.
- Migration: pytest/plugin/Python upgrades can change import mode, async fixtures, warnings and collection.
- Negative-space: collected/passed unit tests do not prove the production entrypoint or integration wiring used those paths.
- False-positive guard: xfail can intentionally document a known unsupported case; distinguish expected debt from silently xpassed behavior.
- Learning motif: retain plugin/import/fixture-order motifs with pytest/Python/plugin versions.



## PACK DOCUMENT: DEPTH5.md

# HTE-Code Framework Pack: pytest

**Baseline:** HTE-Code v2 Depth 5
**Role:** framework-specific augmentation; all other detected packs remain additive.
**Maturity:** seeded
...
**Mechanism:** Fixture scope/autouse/parametrization and teardown order shape hidden state.

**Detection:** build fixture graph.

**Adversarial edge:** session state leak, failed setup.

**Qualification:** repeat/random order.

### monkeypatch

**Mechanism:** Monkeypatch/import cache/environment fixtures can make tests validate a world production never sees.

**Detection:** trace patch scope.

**Adversarial edge:** import-before-patch.

**Qualification:** integration unpatched.
...
# RESOLVED PACK: toolchain.python-packaging

## PACK DOCUMENT: CONCERNS.md

# python-packaging Depth-5 Toolchain Concerns

This pack augments the complete HTE-Code v2 Depth-5 baseline. It must be composed with the actual language, compiler/runtime, platform and repo authority.

## High-signal failure mechanics
- pyproject build-system requirements determine which backend builds the project before ordinary dependencies are considered.
- Source tree, editable install, sdist and wheel can expose different files/import paths/generated metadata.
- Environment markers and wheel tags select dependencies/artifacts by Python version, OS, architecture and implementation.
- Unpinned requirements can resolve differently over time; constraints/lock tooling must be identified rather than assumed.
- Build isolation intentionally uses a separate environment and can reveal undeclared build dependencies hidden by developer machines.
- Native extensions add compiler/ABI/system-library requirements and wheel compatibility constraints.
- Editable installs differ by backend and can mask missing package-data or generated files.
- Namespace packages/package discovery can omit or merge modules unexpectedly.
- requirements files, extras and optional dependency groups encode different runtime capability sets.
- pip cache/index/mirror/config and local path/git dependencies are part of reproducibility.
- Importing from repository cwd can accidentally load source instead of the installed wheel being qualified.
- Qualification builds sdist/wheel, installs into a clean environment and tests the installed artifact on required Python/platform combinations.


## PACK DOCUMENT: DEPTH5.md

# HTE-Code Toolchain Pack: python-packaging
...
**Mechanism:** venv/interpreter, sys.path, editable installs and multiple package managers can import different code than source tree suggests.

**Detection:** record python -m pip, sys.executable, module __file__.

**Adversarial edge:** editable vs wheel, cwd change.

**Qualification:** build wheel + install isolated + tests.

### metadata

**Mechanism:** pyproject build backend, extras, entry points and package-data determine shipped artifact.

**Detection:** inspect wheel/sdist contents/metadata.

**Adversarial edge:** missing data, optional extra.

**Qualification:** twine/check + installed artifact smoke.
...
# Full Coverage Addendum
- **Undefined/implementation-defined:** package-index availability/order, platform wheel selection, system compiler/native-library discovery, editable-install backend and build isolation details are tool/backend/environment scoped even when pyproject metadata is valid.


## PACK DOCUMENT: GAPS.md

# Python Packaging Depth-5 Gap Closure

## Syntax, metadata types, and build phases
- pyproject.toml, setup.cfg/setup.py, requirements files and lock formats are separate grammars with distinct precedence and tooling support.
- Package metadata types are string/list/marker/specifier schemas, not Python runtime types. PEP 440 versions, environment markers, extras and direct references have their own parsing/coercion rules.
- PEP 517 build isolation runs a build backend in a generated environment. Source import success does not prove sdist/wheel build or installed-wheel behavior.

## Lifetime, serialization, performance, instrumentation
- Virtualenv/interpreter/site-packages lifetime and editable-install mechanisms determine which source is actually imported.
- Wheel/sdist metadata, RECORD, entry points, package data and generated files are serialized distribution contracts.
- Resolver backtracking, source builds, native compilation and large dependency graphs are performance and timeout surfaces.
- Instrumentation should record interpreter, pip/uv/poetry/backend version, active index URLs, lock/hash mode, resolved wheel tags and module __file__ provenance.

## Failure signatures, UB/implementation dependence, guards
- Common production symptoms: editable install masks missing package data, wheel imports a different file than checkout, build isolation lacks a dependency available globally, Windows has no matching native wheel, entry point exists in metadata but target module fails at runtime.
- Native extension build behavior is compiler/ABI/platform-defined; packaging success on one tag does not qualify another.
- Migration includes Python version, manylinux/musllinux/macOS tags, build-backend changes, metadata deprecations, resolver behavior and lockfile schema.
- False-positive guard: 'dependency conflict' from an optional/dev extra is not necessarily a runtime product conflict; reproduce the actual selected extras/markers.
- Negative-space check: dependency declaration or successful resolution is not proof the imported module/version/path matches expectation.
- Learning motifs should retain resolver/backend/interpreter/platform tuple for recurring install/build/import failures.
